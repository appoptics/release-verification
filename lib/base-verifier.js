...
class BaseVerifier {
        ...
        .then(r => {
          return this.verify(version)
            ...
            .catch(e => {
              ...
              versions[ix] = {version, status: 'fatal', error: e};
            })
        })
  ...
  async verify (version) {
    ...
    // getPkgUrl(), getPkgTarget() must be implemented by the package-specific super-class
    const npmTarballUrl = this.getPkgUrl(version);
    ...
    if (!this.options.simulate) {
      const command = this.getPkgUnpackCommand(pkgTarget, this.unpackedPkgDir);
  ...
  extractDifferences (result, options = {}) {
    if (result.error && result.error.code === 1) {
      ...
      const differences = lines.filter(filter).filter(excludeFilter);
