...
async function makeRubyVerifier (packageName, options = {}) {
  ...
  const versions = await rubyVerifier.initialize();
...
class RubyVerifier extends BaseVerifier {
  ...
  getPkgUnpackCommand (pkgTarget) {
    return `node lib/unpack-gem ${pkgTarget} ${this.pkgDir}/unpacked-gem ${this.pkgDir}/${this.unpackedPkgDir}`;
  }
...
if (!module.parent) {
  (async function tester () {
    const nv = await makeRubyVerifier('appoptics_apm');
