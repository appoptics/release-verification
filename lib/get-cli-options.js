...
const optimistOptions = {
  ...
  'k': {
    alias: 'token',
    describe: 'the github access token to use',
  },
  'K': {
    alias: 'otp',
    describe: 'the one-time password required for 2fa',
  },
  't': {
    alias: 'tag-template',
    describe: 'specify tag template for github repository tags [default is repo-dependent]',
    default: ''
  },
  'd': {
    alias: 'differences',
...
function getOptions (options = {}) {
  ...
  if (options.configFile) {
    ...
    if (configFile) {
      try {
        defaults = require(path.resolve(configFile));
      } catch (e) {
